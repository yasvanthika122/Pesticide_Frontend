...
const BASE_URL = 'https://pesticide-backend.onrender.com';
...
// New functions for counting customers
export const countTotalCustomers = async () => {
    try {
        const response = await axios.get(`${BASE_URL}/totalRegisteredCustomers`);
...
export const countTotalCustomersforlogin = async () => {
    try {
        const response = await axios.get(`${BASE_URL}/totalLoggedInCustomers`);
...
// New function for fetching total product quantity in a customer's cart
export const getTotalProductQuantityByCustomer = (sellerId) => async (dispatch) => {
    dispatch(getRequest());

    try {
        const result = await axios.get(`${BASE_URL}/total-cart-quantity/${sellerId}`);
        if (result.data) {
